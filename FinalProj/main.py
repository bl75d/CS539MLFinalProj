...
def main(args):
    ...
    if args.prepare:
        ...
        X_train,X_test,Y_train,Y_test = process(stockDict,labelDict,args.series_length)
    ...
    if args.eval:
        ...
        mdl.compile_model(model)
        res=mdl.evaluate(model, np.asarray(X_test), np.asarray(Y_test), verbose=args.verbose)
        return res
