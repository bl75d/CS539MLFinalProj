...
def training_accuracy(csv, directory, prefix=""):
    def on_resize(event):
    ...
    plt.savefig("{}/{}Training.png".format(directory, prefix), bbox_inches='tight',pad_inches = .10)


def testing_accuracy(data, num_epochs=500, directory="", prefix=""):
    def on_resize(event):
        fig.tight_layout(pad=0.0)
        fig.canvas.draw()
    ...
    plt.yticks(np.arange(0,1.2,.1))
    plt.xlabel('Num Epochs', size=label_font)
    plt.ylabel('Accuracy, Error', size=label_font)
    plt.title('Testing - Results verus Epochs', size=title_font)

    plt.savefig("{}/{}Testing.png".format(directory,prefix), bbox_inches='tight',pad_inches = 0)